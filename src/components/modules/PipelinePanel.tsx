  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden ${className}`}>
      <header className="px-5 py-4 border-b border-border/50 shrink-0">
        ...
        <p className="text-xs text-muted-foreground">Простой канбан для отслеживания сделок</p>
      </header>
      ...
      <div className="px-5 py-3 border-b border-border/30 grid grid-cols-1 md:grid-cols-[1fr,1fr,140px,auto] gap-2 shrink-0">
        ...
        <button onClick={addDeal} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium btn-tactile flex items-center gap-1.5"><Plus className="w-4 h-4" />Добавить</button>
      </div>
            ...
            return (
              <div key={stage.id} className={`rounded-xl border border-border/40 bg-gradient-to-b ${stage.color} p-2 min-h-[200px]`}>
                <div className="flex items-center justify-between px-1 mb-2">
                  ...
                  <span className="text-[10px] text-muted-foreground bg-background/40 px-1.5 rounded-full">{items.length}</span>
                </div>
                <div className="space-y-2">
                  {items.map((d) => (
                    <div key={d.id} className="glass-card border border-border/50 rounded-lg p-2.5 group">
                      ...
                      <div className="flex items-center justify-between mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="flex gap-1">
                          <button onClick={() => move(d.id, -1)} className="p-1 rounded hover:bg-accent/50 text-muted-foreground"><ChevronLeft className="w-3 h-3" /></button>
                          <button onClick={() => move(d.id, 1)} className="p-1 rounded hover:bg-accent/50 text-muted-foreground"><ChevronRight className="w-3 h-3" /></button>
                        </div>
                        <button onClick={() => remove(d.id)} className="p-1 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </div>
                  ))}
                </div>
