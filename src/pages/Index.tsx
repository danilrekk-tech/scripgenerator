...
import {
  ...
  Save, Trash2, SlidersHorizontal, Palette, User, LogOut,
  Wrench, Menu, Star, Settings, ChevronRight,
} from "lucide-react";
...
type MobileTab = "config" | "output" | "armory" | "display-settings" | "audit" | "objections" | "simulator" | "services" | "history" | "favorites";
type DesktopPanel = "main" | "audit" | "objections" | "simulator" | "services" | "history" | "favorites" | "settings";
...
export default function Index() {
  ...
  const handleCompanionGenerate = useCallback(
    (type: "objections" | "arguments" | "benefits" | "dozim") => {
      ...
      const typeLabels = { objections: "возможные возражения клиента и ответы на них", arguments: "дополнительные аргументы в пользу покупки", benefits: "конкретные выгоды, которые получит клиент", dozim: "фразы для дожима и закрытия сделки" };
      const companionContext = `ОСНОВНОЙ СКРИПТ (уже сгенерирован, будет использоваться в том же разговоре):\n${script}\n\nЗАДАЧА: Сгенерируй ${typeLabels[type]}. Это дополнение к основному скрипту выше — они будут использоваться в ОДНОМ И ТОМ ЖЕ разговоре. Не повторяй то, что уже есть в основном скрипте.`;
      setIsGenerating(true);
      setScript((prev) => prev + "\n\n---\n\n");
      setPendingHistorySave(true);
      const svcContext = getServiceContext(config.service);
  ...
  if (!isMobile) {
    return (
      <div className="flex h-screen overflow-hidden">
        {desktopPanel === "main" && (
          <ConfigSidebar
            config={config} onChange={setConfig} onGenerate={() => generate()} isGenerating={isGenerating}
            serviceNames={serviceNames} transcriberUrl={appSettings.transcriberUrl}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="glass-panel border-b border-border/50 px-4 py-2 flex items-center justify-between gap-2 shrink-0 z-10">
            ...
            <div className="flex items-center gap-1 shrink-0">
              {desktopPanel === "main" && (
                <>
                  <div className="relative">
                    <button onClick={() => setShowPresetSave(!showPresetSave)} className="p-2 rounded-lg hover:bg-accent/50 text-muted-foreground hover:text-foreground transition-colors" title="Пресеты">
              ...
              {user ? (
                <button onClick={signOut} className="p-2 rounded-lg hover:bg-accent/50 text-muted-foreground hover:text-foreground transition-colors" title="Выйти">
                  <LogOut className="w-4 h-4" />
                </button>
              ) : (
                <button onClick={() => setShowAuthDialog(true)} className="p-2 rounded-lg hover:bg-accent/50 text-muted-foreground hover:text-foreground transition-colors" title="Войти">
          ...
          <div className="flex flex-1 min-h-0">
            {desktopPanel === "main" && (
              <>
                {showDesktopSettings && (
                  <div className="w-80 shrink-0 border-r border-border/50 glass-panel p-6 overflow-y-auto">
                ...
                <ScriptOutput
                  script={script} isGenerating={isGenerating} mode={config.mode} displaySettings={displaySettings}
            ...
            {desktopPanel === "audit" && (
              <div className="flex-1 glass-panel m-2 rounded-xl overflow-hidden">
            ...
            {desktopPanel === "favorites" && (
              <div className="flex-1 glass-panel m-2 rounded-xl overflow-hidden">
                <FavoritesPanel favorites={favorites} onLoad={handleHistoryLoad} onRemove={removeFavorite} />
              </div>
            )}
            {desktopPanel === "settings" && (
              <div className="flex-1 glass-panel m-2 rounded-xl overflow-hidden">
                <AppSettingsPanel
                  ...
                  user={user} onSignIn={() => setShowAuthDialog(true)} onSignOut={signOut}
                />
              </div>
            )}
          </div>
  ...
  const toolItems = [
    ...
    { tab: "audit" as MobileTab, label: "Аудит сайта", icon: <Globe className="w-5 h-5" />, desc: "SEO-аудит и персонализированный скрипт" },
    { tab: "objections" as MobileTab, label: "Тренажёр возражений", icon: <Zap className="w-5 h-5" />, desc: "Генерация возражений для тренировки" },
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <header className="glass-panel border-b border-border/50 flex items-center justify-between px-4 py-3 shrink-0 z-10">
        ...
        <div className="flex items-center gap-1">
          {user ? (
            <button onClick={signOut} className="p-2 rounded-lg hover:bg-accent/50 text-muted-foreground"><LogOut className="w-4 h-4" /></button>
          ) : (
      ...
      <div className="flex-1 overflow-hidden">
        {mobileTab === "config" && (
          <ConfigSidebar config={config} onChange={setConfig} onGenerate={() => generate()} isGenerating={isGenerating} serviceNames={serviceNames} className="!w-full !border-r-0 h-full glass-panel" transcriberUrl={appSettings.transcriberUrl} />
        )}
        {mobileTab === "output" && (
          <ScriptOutput script={script} isGenerating={isGenerating} mode={config.mode} displaySettings={displaySettings} className="h-full" onCompanionGenerate={handleCompanionGenerate} isFavorite={isFavorite(script)} onToggleFavorite={handleToggleFavorite} />
        )}
      ...
      <nav className="glass-panel border-t border-border/50 flex items-center justify-around py-2 shrink-0 z-10">
        ...
        <MobileNavBtn active={["armory", "audit", "objections", "simulator"].includes(mobileTab)} onClick={() => setShowToolsSheet(true)} icon={<Wrench className="w-5 h-5" />} label="Инструменты" />
        ...
        <SheetContent side="bottom" className="glass-panel border-border/50 rounded-t-2xl">
          ...
          <div className="flex flex-col gap-2 py-4">
            {toolItems.map((item) => (
              <button
                key={item.tab}
                onClick={() => { setMobileTab(item.tab); setShowToolsSheet(false); }}
                className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-accent/50 transition-colors text-left"
              >
        ...
        <SheetContent side="bottom" className="glass-panel border-border/50 rounded-t-2xl">
          ...
          <div className="flex flex-col gap-2 py-4">
            {menuItems.map((item) => (
              <button
                key={item.tab}
                onClick={() => { setMobileTab(item.tab); setShowMenuSheet(false); }}
                className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-accent/50 transition-colors text-left"
              >
                <div className="text-primary">{item.icon}</div>
                <div className="text-sm font-medium text-foreground">{item.label}</div>
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 overflow-y-auto">
        <div className="text-center">
          ...
          <h3 className="text-base font-medium text-foreground mb-1">Нет избранных скриптов</h3>
          <p className="text-xs text-muted-foreground">Нажмите ★ на сгенерированном скрипте</p>
        </div>
      </div>
    );
  }
        ...
        {favorites.map((fav) => (
          <div key={fav.id} className="glass-card border border-border/50 rounded-xl p-4 group">
            <div className="flex items-start justify-between gap-2 mb-2">
              <div>
                <span className="text-[10px] uppercase tracking-wider text-primary font-medium">{fav.mode}</span>
                <span className="text-[10px] text-muted-foreground ml-2">{fav.service}</span>
              </div>
          ...
          {user ? (
            <div className="glass-card border border-border/50 rounded-xl p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Данные синхронизируются с облаком</p>
              </div>
