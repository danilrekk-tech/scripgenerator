...
КРИТИЧЕСКИ ВАЖНО: Если в контексте указана информация об услуге (описание и ключевые пункты), используй ТОЛЬКО эти данные. НЕ выдумывай.`;
  ...
  switch (mode) {
    ...
    case "live-call":
      return `${base}\n\nТы — СУФЛЁР во время живого звонка. Менеджер продаёт digital-услуги. Дай КРАТКИЙ и ТОЧНЫЙ ответ (2-4 предложения), который менеджер может сразу использовать. Не объясняй — просто дай готовую фразу.\n${getToneDescription(tone)}`;

    case "pre-call-brief":
      return `${base}\n\nТы генерируешь ПРЕ-СЕЙЛ БРИФ — подготовку к звонку клиенту. На основе информации о компании определи:\n1. Потенциальные боли и потребности\n2. Ключевые вопросы для выявления потребностей\n3. Подходящие аргументы\n4. Вероятные возражения и ответы\n5. Стратегию первого контакта\n${priceInstr}`;

    case "objection-library":
      return `${base}\n\nСгенерируй библиотеку возражений с ответами. Ответ строго в формате JSON массив без markdown.`;
    ...
    case "success-story":
      return `${base}\n\nТы генерируешь ИСТОРИЮ УСПЕХА (кейс) на основе описания проекта. Превращаешь сухие данные в убедительный продающий нарратив.\n\nСтруктура:\n## Проблема клиента\n## Что было сделано\n## Результаты (с цифрами)\n## Цитата клиента (придумай реалистичную)\n## Вывод\n\nСтиль: живой, конкретный, с цифрами и деталями.`;

    case "warmup-funnel":
      return `${base}\n\nТы генерируешь ВОРОНКУ ПРОГРЕВА — последовательность из 5-7 касаний с разными каналами и контентом. Для каждого шага:\n- День и канал (звонок / email / мессенджер / SMS)\n- Цель касания\n- Полный текст\n- Триггер перехода на следующий шаг\n${getToneDescription(tone)}\n${priceInstr}\n${nameInstr}`;
    ...
    case "client-simulation":
      return `Ты играешь роль реального клиента в телефонном разговоре. Ты НЕ ИИ и никогда об этом не упоминаешь. Отвечай ТОЛЬКО как клиент, живой разговорной речью, 1-3 предложения.

ПРАВИЛА РЕАЛИСТИЧНОСТИ:
- Держи характер, настроение, бюджет и уровень возражений из контекста — не «плавай» от реплики к реплике.
- Помни всё, что менеджер уже говорил: не задавай повторно вопросы, на которые получил ответ, и ссылайся на его слова («вы же сказали, что...»).
- Реагируй именно на последнюю реплику менеджера. Слабый или общий ответ — усиливай сомнение; конкретный ответ с цифрами и выгодой — постепенно теплей.
- Иногда добавляй бытовые детали своей компании (сроки, прошлый неудачный опыт, конкуренты, согласование с руководством).
- Не выдавай сразу все возражения: по одному, естественно. Не подсказывай менеджеру, как продавать.
- Если менеджер грубит, давит или врёт — реагируй холоднее, вплоть до завершения разговора.
- Если менеджер закрыл ключевые сомнения и предложил конкретный следующий шаг — соглашайся на него, не тяни диалог искусственно.
- Без markdown, без списков, без пояснений в скобках — только прямая речь клиента.`;
...
function getUserPrompt(mode: string, service: string, situation: string, tone: string, context: string, transcript: string, emailSubtype: string, emailObjection: string, extraParams?: Record<string, string>) {
  ...
  switch (mode) {
    ...
    case "live-call":
      return `${context || `Клиент на линии. Услуга: ${service}.`}`;
    case "pre-call-brief":
    ...
    case "manipulation-detector":
      return `Проанализируй диалог на манипуляции.\nУслуга: ${service}${ctx}`;
    case "success-story":
      return `Создай историю успеха.\nУслуга: ${service}${ctx}`;
    case "warmup-funnel":
      return `Создай воронку прогрева.\nУслуга: ${service}${ctx}`;
    case "touch-chain":
